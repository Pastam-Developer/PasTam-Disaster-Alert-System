...
@section('content')
    ...
    <main class="max-w-full flex-1 p-8">
        ...
        <!-- Table Controls -->
        <form method="GET" action="{{ route('announcements.index') }}" class="bg-white rounded-xl shadow border mb-6 p-4">
            <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                <div class="flex items-center space-x-4">
                    <div class="relative">
                        <i class="fas fa-search absolute left-3 top-3 text-gray-400"></i>
                        <input type="text" name="search" placeholder="Search announcements..." value="{{ $search }}" class="pl-10 pr-4 py-2 border rounded-lg w-64">
                    </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr data-id="{{ $announcement->id }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" class="rounded announcement-checkbox" value="{{ $announcement->id }}">
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    @if($announcement->image)
                                    <img src="{{ asset('storage/' . $announcement->image) }}" alt="{{ $announcement->title }}" class="w-12 h-12 rounded-lg object-cover mr-4">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500 truncate max-w-xs">{{ Str::limit($announcement->description, 100) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-3 py-1 rounded-full text-xs font-medium {{ $announcement->getStatusBadgeClass() }}">
                                    {{ ucfirst($announcement->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div class="font-medium">{{ $announcement->getFormattedStartDate() }}</div>
                                @if($announcement->end_at)
                                <div class="text-gray-500 text-xs">to {{ $announcement->getFormattedEndDate() }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    <button class="text-primary hover:text-secondary edit-announcement" data-id="{{ $announcement->id }}">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="text-danger hover:text-red-700 delete-announcement" data-id="{{ $announcement->id }}">
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                <div class="flex flex-col md:flex-row justify-between items-center">
                    <div class="text-sm text-gray-500 mb-4 md:mb-0">
                        Showing <span class="font-medium">{{ $announcements->firstItem() }}</span> to 
                        <span class="font-medium">{{ $announcements->lastItem() }}</span> of 
                        <span class="font-medium">{{ $announcements->total() }}</span> announcements
                    </div>
