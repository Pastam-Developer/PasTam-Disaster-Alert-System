<aside class="sidebar bg-[#1E88E5] w-64 fixed inset-y-0 left-0 border-r border-blue-300 shadow-xl z-40" id="sidebar">
    ...
    <div class="flex items-center justify-between h-16 px-6 border-b border-blue-300 bg-[#1565C0]">
        <div class="flex items-center space-x-2">
            <img src="{{ asset('images/logo.png') }}" alt="Disaster Alert" class="h-8 w-8 rounded-full bg-white p-1">
    ...
    <div class="h-full overflow-y-auto px-4 py-6 space-y-2">
        <ul class="space-y-1 text-white font-medium">
            ...
            <!-- Dashboard -->
            <li>
                <a href="{{ route('dashboard') }}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition">
            ...
            <!-- Real-Time Alerts -->
            <li>
                <a href="{{ route('incidents.index') }}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition">
            ...
            <!-- Safe Zones -->
            <li>
                <a href="{{route('announcements.index')}}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition">
            ...
            <!-- Admin: Manage Alerts -->
            <li class="border-t border-blue-300 pt-3 mt-3">
                <p class="text-xs uppercase text-blue-100 font-semibold px-3 mb-2">Admin Controls</p>

                <a href="{{route('incidents.index')}}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition">
                    <i class="fa-solid fa-bullhorn w-5"></i>
                    <span class="ml-3">Post New Alert</span>
                </a>

                <a href="" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition">
                    <i class="fa-solid fa-users-gear w-5"></i>
                    <span class="ml-3">Manage Officials</span>
                </a>
            </li>
            @endif
            @endauth --}}
    ...
    <div class="absolute bottom-0 left-0 w-full p-5 bg-[#1565C0] border-t border-blue-300">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm font-semibold text-white">{{ auth()->user()->first_name ?? 'User' }}</p>
                    <p class="text-xs text-blue-100">{{ strtoupper(auth()->user()->role ?? 'Guest') }}</p>
                </div>
            </div>
