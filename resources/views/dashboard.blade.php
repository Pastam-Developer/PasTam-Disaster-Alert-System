...
@section('title', 'Incident Management Dashboard')
...
<div class="container max-w-full px-4 py-8">
        ...
        <div class="dashboard-card">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <div class="text-2xl font-bold text-gray-900">{{ $incidentStats['overdue'] }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="dashboard-card">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <div class="text-2xl font-bold text-gray-900">{{ $incidentStats['in_progress'] }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="dashboard-card">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <div class="text-2xl font-bold text-gray-900">{{ end($incidentTrends['data']) }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="dashboard-card stat-card">
            ...
            <div class="stat-number text-blue-600">{{ $incidentStats['total'] }}</div>
        ...
        <div class="dashboard-card stat-card">
            ...
            <div class="stat-number text-yellow-600">{{ $incidentStats['pending'] }}</div>
        ...
        <div class="dashboard-card stat-card">
            ...
            <div class="stat-number text-green-600">{{ $incidentStats['resolved'] }}</div>
        ...
        <div class="dashboard-card stat-card">
            ...
            <div class="stat-number text-purple-600">{{ $incidentStats['avg_response_time'] }} min</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="font-medium text-gray-900">{{ $incident['report_id'] }}</div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center">
                                    <span class="priority-dot" style="background-color: {{ $incident['priority_color'] }}"></span>
                                    <span class="font-medium">{{ ucfirst($incident['urgency_level']) }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                @php
                                ...
                                @endphp
                                <span class="incident-badge {{ $statusColors[$incident['status']] ?? 'bg-gray-100 text-gray-800' }}">
        ...
        <div class="dashboard-card">
            ...
            <div class="space-y-4">
                ...
                <div class="p-4 rounded-lg {{ $announcement->priority === 'high' ? 'announcement-high-priority' : 'announcement-card' }}">
                    <div class="flex justify-between items-start">
                        ...
                        <span class="incident-badge {{ $announcement->getStatusBadgeClass() }}">
                            {{ ucfirst($announcement->status) }}
                        </span>
                    </div>
                    <div class="mt-3 flex items-center justify-between text-xs text-gray-500">
                        <span>Starts: {{ $announcement->getFormattedStartDate() }}</span>
                        @if($announcement->end_at)
                        <span>Ends: {{ $announcement->getFormattedEndDate() }}</span>
                        @endif
                    </div>
                </div>
                @empty
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(trendCtx, {
        type: 'line',
        data: {
            labels: @json($incidentTrends['months']),
            datasets: [{
                label: 'Incidents',
                data: @json($incidentTrends['data']),
                borderColor: '#3B82F6',
        ...
        options: {
            ...
            scales: {
                y: {
                    ...
                    ticks: {
                        stepSize: Math.max(1, Math.ceil(Math.max(...@json($incidentTrends['data'])) / 10))
                    }
                },
                x: {
                    grid: {
    ...
    new Chart(typeCtx, {
        type: 'doughnut',
        data: {
            labels: @json($incidentByType['labels']),
            datasets: [{
                data: @json($incidentByType['data']),
                backgroundColor: @json($incidentByType['colors']),
                borderWidth: 2,
                borderColor: '#ffffff'
            }]
        },
    ...
    new Chart(urgencyCtx, {
        type: 'bar',
        data: {
            labels: @json($incidentByUrgency['labels']),
            datasets: [{
                label: 'Incidents',
                data: @json($incidentByUrgency['data']),
                backgroundColor: @json($incidentByUrgency['colors']),
                borderColor: @json($incidentByUrgency['colors']),
                borderWidth: 1,
                borderRadius: 6,
                borderSkipped: false,
            }]
    ...
    new Chart(statusCtx, {
        type: 'pie',
        data: {
            labels: @json($incidentStatusDistribution['labels']),
            datasets: [{
                data: @json($incidentStatusDistribution['data']),
                backgroundColor: @json($incidentStatusDistribution['colors']),
                borderWidth: 2,
                borderColor: '#ffffff'
            }]
        },
